...
export function Step4Export({ config }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {error ? (
        <div className="flex items-center gap-3 rounded-none-none-none border border-red-800/60 bg-red-950/20 p-4">
        ...
        <>
          {/* Descriptor box */}
          <div className="rounded-none-none-none border border-[#1e2640] bg-[#121626]/40 p-6 flex flex-col items-center space-y-6 relative">
            <div 
              className="bg-white p-4 rounded-none-none-none shadow-sm relative group cursor-pointer"
              onClick={() => setShowFullscreenQR(true)}
            >
              <QRCode value={descriptor} size={192} className="w-48 h-48 transition-opacity group-hover:opacity-80" />
              <button 
                className="absolute bottom-2 right-2 bg-zinc-900/80 p-1.5 rounded-none-none-none text-white opacity-0 group-hover:opacity-100 transition-opacity backdrop-blur-sm"
            ...
            <div className="w-full space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={copy}
                  className="flex items-center gap-2 px-3 py-1.5 rounded-none-none-none border border-[#6366f1]/40 bg-[#6366f1]/10 text-[#818cf8] text-xs font-bold hover:bg-[#6366f1]/20 hover:text-white transition-all shadow-sm"
                >
                  {copied ? (
                    <>
              ...
              <p className="text-xs font-mono text-zinc-350 break-all bg-zinc-950 p-3 rounded-none-none-none border border-[#1e2640] leading-relaxed max-h-24 overflow-y-auto">
                {descriptor}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            {[
              ...
              <div
                key={item.label}
                className="rounded-none-none-none border border-[#1e2640] bg-[#181d33]/20 p-3 text-center"
              >
          ...
          <div className="flex gap-3 rounded-none-none-none border border-[#1e2640] bg-[#121626]/20 p-4">
          ...
          {addresses.length > 0 && (
            <div className="rounded-none-none-none border border-[#1e2640] bg-zinc-950/40">
              <button
          ...
          <div className="flex gap-3 flex-col sm:flex-row flex-wrap">
            <Button
              onClick={download}
              className="flex-1 bg-[#6366f1] hover:bg-[#4f46e5] text-white font-semibold h-12 text-sm shadow-[0_0_15px_rgba(99,102,241,0.15)] min-w-[200px]"
            >
            ...
            <Button
              onClick={downloadPdf}
              className="flex-1 bg-transparent border border-[#6366f1] text-[#818cf8] hover:bg-[#6366f1]/10 font-semibold h-12 text-sm min-w-[200px]"
            >
            ...
            <Button
              onClick={shareNative}
              className="flex-1 bg-transparent border border-zinc-700 text-zinc-300 hover:bg-zinc-800 font-semibold h-12 text-sm min-w-[200px]"
            >
              <Share2 className="w-4 h-4 mr-2" />
              Compartir Texto (Nativo)
            </Button>
          </div>
        </>
      )}
      ...
      {showFullscreenQR && (
        <div 
          className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/90 backdrop-blur-sm animate-scaleIn p-4"
          onClick={() => setShowFullscreenQR(false)}
        >
          <div 
            className="bg-white p-8 rounded-none-none-none relative shadow-2xl animate-slideUp max-w-[90vw] max-h-[90vh] flex items-center justify-center"
            onClick={(e) => e.stopPropagation()}
          >
            <button 
              className="absolute -top-4 -right-4 bg-zinc-900 text-white p-2 rounded-none-none-full hover:bg-zinc-800 transition-colors shadow-lg border border-zinc-700"
              onClick={() => setShowFullscreenQR(false)}
            >
              <X className="w-5 h-5" />
            </button>
            <QRCode value={descriptor} size={Math.min(window.innerWidth * 0.8, window.innerHeight * 0.8, 500)} />
          </div>
        </div>
      )}
    </div>
